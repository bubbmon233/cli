...
| `--inline '<json>'` | 否 | 高级用法：手动指定内嵌图片 CID 映射。多个 inline 图片请重复传 `--inline`，每次只放一个 JSON object，并用单引号包住：`'{"cid":"mycid","file_path":"./logo.png"}'`。推荐直接在 `--body` 中使用 `<img src="./path" />`（自动解析）。在 body 中用 `<img src="cid:mycid">` 引用。不可与 `--plain-text` 同时使用 |

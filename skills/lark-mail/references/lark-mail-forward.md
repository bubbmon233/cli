...
| `--attach <path>` | 否 | 附件文件路径。多个附件请重复传 `--attach`，每次只放一个相对路径；路径包含空格、括号或中文时用引号包住；按传入顺序追加在原邮件附件之后。当附件导致 EML 总大小超过 25 MB 时，超出部分自动上传为超大附件（HTML 邮件插入下载卡片，纯文本邮件追加下载链接），单个文件上限 3 GB |

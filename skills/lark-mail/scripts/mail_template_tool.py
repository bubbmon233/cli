...
CATALOG_PATH = REFERENCES_DIR / "mail-template-catalog.md"
DEFAULT_INDEX_PATH = REFERENCES_DIR / "mail-template-index.json"
...
def tokenize_query(value: str) -> list[str]:
    ...
    synonym_tokens = {
        # Mail scene synonyms
        "通告": ["notify", "notification", "announcement"],
        "通知": ["notify", "notification"],
        "公告": ["notify", "notification"],
        "周报": ["weekly", "report"],
        "月报": ["monthly", "report"],
        "汇报": ["report"],
        "决策": ["decision", "approval", "review"],
        "评审": ["decision", "review"],
        "审批": ["decision", "approval"],
        "故障": ["incident", "postmortem"],
        "复盘": ["postmortem", "review"],
        "事故": ["incident"],
        "启动": ["kickoff", "launch"],
        "立项": ["kickoff"],
        "欢迎": ["welcome"],
        "入职": ["welcome", "onboarding"],
        "新人": ["welcome", "onboarding"],
        "资讯": ["newsletter", "digest"],
        "日报": ["newsletter", "daily"],
        "新闻": ["newsletter", "news"],
        "速递": ["newsletter", "digest"],
        "同步": ["sync"],
        "纪要": ["followup", "notes"],
        "会议": ["meeting", "followup"],
        "升级": ["escalation"],
        "求助": ["escalation", "help"],
        "发版": ["release", "changelog"],
        "上线": ["release", "launch"],
        # Formality synonyms
        "正式": ["formal"],
        "商务": ["formal"],
...
def get_template_path(entry: dict[str, Any]) -> Path:
    return TEMPLATES_DIR / f'{entry["template_id"]}.html'
...
def print_usage() -> None:
    usage = [
        "Usage:",
        "  python3 mail_template_tool.py build-index [--out <path>]",
        "  python3 mail_template_tool.py search --query <text> [--scene notify|weekly|decision|incident|kickoff|welcome|newsletter] [--formality formal|casual|urgent] [--limit 3]",
        "  python3 mail_template_tool.py summarize --template <template-id|path> (--section <label>)",
        "  python3 mail_template_tool.py extract --template <template-id|path> (--section <label>) [--with-summary] [--out <path>]",
    ]

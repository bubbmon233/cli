...
| [`+watch`](references/lark-mail-watch.md) | Watch for incoming mail events via the unified event framework (requires scope mail:event and event mail.user_mailbox.event.message_received_v1 enabled). Run with --print-output-schema to see per-format field reference before parsing output. |

...
| [`+messages`](references/lark-mail-messages.md) | Use when reading full content for multiple emails by message ID. Internally calls `messages.batch_get`; OAPI Meta/gateway config allows up to 20 IDs per call, so CLI chunks execution by 20 IDs and merges output. |

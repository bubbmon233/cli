...
var MailReplyAll = common.Shortcut{
	...
	Flags: []common.Flag{
		...
		{Name: "to", Type: "string_array", Desc: "Additional To address. Repeat --to once per recipient; appended to original recipients."},
		{Name: "cc", Type: "string_array", Desc: "Additional CC email address. Repeat --cc once per recipient."},
		{Name: "bcc", Type: "string_array", Desc: "BCC email address. Repeat --bcc once per recipient."},
		{Name: "remove", Type: "string_array", Desc: "Address to exclude from the outgoing reply. Repeat --remove once per address."},

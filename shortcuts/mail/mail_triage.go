	...
	Execute: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		default: // "table"
			...
			if mailbox != "me" {
				...
				fmt.Fprintln(runtime.IO().ErrOut, "tip: read full content: single message use mail +message --mailbox "+quotedMailbox+" --message-id id1; multiple messages use mail +messages --mailbox "+quotedMailbox+" --message-ids id1,id2,id3")
			} else {
				fmt.Fprintln(runtime.IO().ErrOut, "tip: read full content: single message use mail +message --message-id id1; multiple messages use mail +messages --message-ids id1,id2,id3")
			}
		}
		return nil
	},

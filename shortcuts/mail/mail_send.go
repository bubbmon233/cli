...
var MailSend = common.Shortcut{
	...
	HasFormat:   true,
	Flags: []common.Flag{
		{Name: "to", Type: "string_array", Desc: "Recipient email address(es). Repeat the flag or separate multiple addresses with commas."},
		...
		{Name: "cc", Type: "string_array", Desc: "CC email address(es). Repeat the flag or separate multiple addresses with commas."},
	...
	Validate: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		if !hasTemplate {
			if err := validateComposeHasAtLeastOneRecipient(to, cc, bcc); err != nil {
				return err
			}
		}
...
// and omitting empty entries. Used for comma-compatible file-path lists such as --attach.

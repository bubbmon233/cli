...
var MailDraftCreate = common.Shortcut{
	...
	HasFormat:   true,
	Flags: []common.Flag{
		{Name: "to", Type: "string_array", Desc: "Optional. Full To recipient list. Repeat the flag or separate multiple addresses with commas. Display-name format is supported. When omitted, the draft is created without recipients (they can be added later via +draft-edit)."},
		...
		{Name: "cc", Type: "string_array", Desc: "Optional. Full Cc recipient list. Repeat the flag or separate multiple addresses with commas. Display-name format is supported."},
		{Name: "bcc", Type: "string_array", Desc: "Optional. Full Bcc recipient list. Repeat the flag or separate multiple addresses with commas. Display-name format is supported."},
		{Name: "plain-text", Type: "bool", Desc: "Force plain-text mode, ignoring HTML auto-detection. Cannot be used with --inline."},
		{Name: "attach", Type: "string_array", Desc: "Optional. Regular attachment file paths (relative path only). Repeat the flag or separate multiple paths with commas. Each path must point to a readable local file."},
		{Name: "inline", Type: "string_array", Desc: "Optional. Inline images as JSON object or array. Repeat the flag to append entries; values are not comma-split. Each entry: {\"cid\":\"<unique-id>\",\"file_path\":\"<relative-path>\"}. Cannot be used with --plain-text."},
	...
	Execute: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		input.Inline, err = normalizeInlineFlagValues(runtime.StrArray("inline"))
		if err != nil {
			return err
		}
